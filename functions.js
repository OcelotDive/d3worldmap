 ...
 function greenOnOff() {
     gc % 2 === 0 ? $('.greenCircle').css('display', 'none') : $('.greenCircle').css('display', 'inline')
     gc++;
 }

 function purpleOnOff() {
     pc % 2 === 0 ? $('.purpleCircle').css('display', 'none') : $('.purpleCircle').css('display', 'inline')
     pc++;
 }

 function orangeOnOff() {
     oc % 2 === 0 ? $('.orangeCircle').css('display', 'none') : $('.orangeCircle').css('display', 'inline')
     oc++;

 }

 function redOnOff() {
     rc % 2 === 0 ? $('.redCircle').css('display', 'none') : $('.redCircle').css('display', 'inline')
     rc++;
 ...
 var keyData = ['Up to 10kg', '10.1kg - 25kg', '25.1kg - 1000kg', 'Over 1000kg'];
 ...
 d3.json("https://ucarecdn.com/8e1027ea-dafd-4d6c-bf1e-698d305d4760/world110m2.json", function (error, topology) {
     ...
     d3.json("https://raw.githubusercontent.com/FreeCodeCamp/ProjectReferenceData/master/meteorite-strike-data.json", function (error, data) {
         ...
         var legend = svg.selectAll('.legend')
             .data(['#61B329', '#551a8b', '#ffa500', '#cc0000'])
             .enter().append('g')
             .attr("class", "legend")
         ...
         var circles = svg.selectAll("circle")
             ...
             .attr('class', function (d) {
                 return (~~d.properties.mass / 1000) <= 10 ? 'greenCircle' : (~~d.properties.mass / 1000) > 10 && (~~d.properties.mass / 1000) <= 25 ? 'purpleCircle' : (~~d.properties.mass / 1000) > 25 && (~~d.properties.mass / 1000) <= 1000 ? 'orangeCircle' : (~~d.properties.mass / 1000) > 1000 ? 'redCircle' : null
             })

         circles.attr("cx", function (d) {

                 return projection([d.properties.reclong, d.properties.reclat])[0] + 10
             })
             .attr("cy", function (d) {
                 return projection([d.properties.reclong, d.properties.reclat])[1] + 5
             })
             .attr("r", function (d) {
                 return (~~d.properties.mass / 1000) <= 10 ? 3 : (~~d.properties.mass / 1000) > 10 && (~~d.properties.mass / 1000) <= 25 ? 3.5 : (~~d.properties.mass / 1000) > 25 && (~~d.properties.mass / 1000) <= 1000 ? 5 : (~~d.properties.mass / 1000) > 1000 ? 16 : null
             })

             .attr("fill", function (d) {
                 return (~~d.properties.mass / 1000) <= 10 ? '#61B329' : (~~d.properties.mass / 1000) > 10 && (~~d.properties.mass / 1000) <= 25 ? '#551a8b' : (~~d.properties.mass / 1000) > 25 && (~~d.properties.mass / 1000) <= 1000 ? '#ffa500' : (~~d.properties.mass / 1000) > 1000 ? '#cc0000' : null
             })
             .style('opacity', 0.6)
             .attr('stroke', '#000')
             .style('z-index', function (d) {
                 return (~~d.properties.mass / 1000) <= 10 ? 9 : (~~d.properties.mass / 1000) > 10 && (~~d.properties.mass / 1000) <= 25 ? 8 : (~~d.properties.mass / 1000) > 25 && (~~d.properties.mass / 1000) <= 1000 ? 7 : (~~d.properties.mass / 1000) > 1000 ? 6 : null
             })
             ...
             .on("mouseover", function (d) {
                 tooltip.transition()
                     .duration(200)
                     .style("opacity", .9);
                 tooltip.html('Name: ' + d["properties"]["name"] + '<br>' + 'Mass: ' + format(d["properties"]["mass"] / 1000) + ' ' + 'kg' + '<br>' + 'Long: ' + d["properties"]["reclong"] + ' ' + 'Lat: ' + d["properties"]["reclong"] + '<br>' + 'Year: ' + d["properties"]["year"].substring(0, 4) + '<br>' + 'Class: ' + d["properties"]["recclass"])
